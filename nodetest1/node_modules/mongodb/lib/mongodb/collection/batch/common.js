...
var mergeBatchResults = function (ordered, batch, bulkResult, err, result) {
    ...
    if (Array.isArray(result.upserted)) {
        ...
        for (var i = 0; i < result.upserted.length; i++) {
            bulkResult.upserted.push({
                index: result.upserted[i].index + batch.originalZeroIndex, _id: result.upserted[i]._id
            });
        }
        ...
        bulkResult.upserted.push({
            index: batch.originalZeroIndex, _id: result.upserted
        });
    }
    ...
    if (Array.isArray(result.writeErrors)) {
        for (var i = 0; i < result.writeErrors.length; i++) {
            ...
            var writeError = {
                index: batch.originalZeroIndex + result.writeErrors[i].index, code: result.writeErrors[i].code, errmsg: result.writeErrors[i].errmsg, op: batch.operations[result.writeErrors[i].index]
            };
...
var mergeLegacyResults = function (_ordered, _op, _batch, _results, _result, _index) {
    ...
    if ((_result.errmsg || _result.err || _result instanceof Error) && _result.wtimeout != true) {
        ...
        var errResult = new WriteError({
            index: _index, code: code, errmsg: errmsg, op: _op
        });
